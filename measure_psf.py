...
def measure_psf(im,ra,dec):
	...
	im.plot(zscale=True)
        ...
        for r,d in zip(ra,dec):
                gfit = im.gauss_fit(center=(d,r),pos_min=(d-2,r-2),pos_max=(d+2,r+2),circular=True,verbose=0,plot=True)
                gfwhm.append(gfit.fwhm[0])
		try:
                	fit = im.moffat_fit(center=gfit.center,pos_min=(d-2,r-2),pos_max=(d+2,r+2),circular=True,verbose=0)
                	n.append(fit.n)
...
if __name__ == "__main__":
	...
	## Load cube and loop over the several wavelengh planes
	c = Cube(args.cube)
